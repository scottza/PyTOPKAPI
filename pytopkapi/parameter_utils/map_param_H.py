...
def create_maps(base_path):
    ...
    ar_cell_label, ar_coorx, ar_coory, ar_lambda, ar_Xc, ar_wwb, ar_tan_beta,\
    ar_tan_beta_channel, ar_L, ar_Ks, ar_theta_r, ar_theta_s, ar_n_o, ar_n_c,\
    ar_cell_down,ar_pVs_t0,ar_Vo_t0,ar_Qc_t0,ar_kc, psi_b, lamda\
    =pm.read_cell_parameters(file_cell_param)

    #Lambda
    image_out=path_out+'field_lambda_channel.png'
    ...
    #WWB
    image_out=path_out+'field_WWB.png'
    field_map(ar_wwb,ar_coorx,ar_coory,X,image_out,'WWB',min_val=min(ar_wwb[ar_wwb>0]),max_val=max(ar_wwb[ar_wwb<200]))
    #Slopes
