...
#                print 'Process memory at %dMB - ndar_Qd_out' %mp
        ...
        varnames = ['ndar_ET_out','ndar_Qs_out','ndar_Vs_cel','ndar_Qo_out',\
        'ndar_Vd_cel','ndar_Qd_inp','ndar_Qd_out']
 ...
 arrays will be mapped on harddrive - long processing times can be expected..'
        ...
        varnms = ['ndar_ET_out','ndar_Qs_out','ndar_Vs_cel','ndar_Qo_out','ndar_Vd_cel','ndar_Qd_inp','ndar_Qd_out']
        groups = ['/',          '/Soil/',     'Soil',       'Overland',   'Dam',        'Dam',        'Dam']
        entris = ['ET_out',     'Qs_out',     'V_s',        'Qo_out',     'V_d',        'Qd_in',      'Qd_out']
...
#==============================================================================
    ...
    ws_hdr = ['ETa','Vs','Qs_in','Qs_out','Qo_in','Qo_out','Qs_in_r','Qo_in_r',\
            'Rain','WB','ERR','SSI']
    um3    = ['(m3/','(m3)' ,'(m3/','(m3/','(m3/','(m3/',
              '(m3/','(m3/','(m3/','(m3/','(m3/','-']
    umm    = ['(mm/','(mm)' ,'(mm/','(mm/','(mm/','(mm/',
              '(mm/','(mm/','(mm/','(mm/','(mm/','-']
...
COLUMN()-9)))-INDIRECT(ADDRESS(ROW(),COLUMN()-1))'
    ...
    for h in xrange(nb_wwb):
        ...
        for l in xrange(n_rec):
            ...
            Qs_in_w[h,l]-Qo_ot_w[h,l]+Qo_in_w[h,l]
            if h == 0:
            ...
            ws_3.write(l+2, nb_var*h+1+3,  Qs_ot_w[h,l])
            ws_3.write(l+2, nb_var*h+1+4,  Qo_in_w[h,l])
            ws_3.write(l+2, nb_var*h+1+5,  Qo_ot_w[h,l])
            ws_3.write(l+2, nb_var*h+1+6,  Qs_in_r[h,l])
            ws_3.write(l+2, nb_var*h+1+7,  Qo_in_r[h,l])
#            try:
...
#                raise ValueError('here you go.. that is wrong')
            ...
            ws_55.write(l+2, nb_var*h+1+3,  Qs_ot_w[h,l]/A_2_mm_w[h])
            ws_55.write(l+2, nb_var*h+1+4,  Qo_in_w[h,l]/A_2_mm_w[h])
            ws_55.write(l+2, nb_var*h+1+5,  Qo_ot_w[h,l]/A_2_mm_w[h])
            ws_55.write(l+2, nb_var*h+1+6,  Qs_in_r[h,l]/A_2_mm_w[h])
            ws_55.write(l+2, nb_var*h+1+7,  Qo_in_r[h,l]/A_2_mm_w[h])
#            try:
...
#                raise ValueError('here you go.. that is wrong')
        ...
        for l in xrange(len(D_mon)):
            ...
            ws_4.write(l+2, nb_var*h+1+3,  Qs_ot_m_w[h,l])
            ws_4.write(l+2, nb_var*h+1+4,  Qo_in_m_w[h,l])
            ws_4.write(l+2, nb_var*h+1+5,  Qo_ot_m_w[h,l])
            ws_4.write(l+2, nb_var*h+1+6,  Qs_in_m_r[h,l])
            ws_4.write(l+2, nb_var*h+1+7,  Qo_in_m_r[h,l])
            ws_4.write(l+2, nb_var*h+1+8,  Pt_m_w[h,l])
            ws_4.write(l+2, nb_var*h+1+9,  wb_tmp_m)
            if l:
                ws_4.write_formula(l+2, nb_var*h+1+10, er_f)
            ws_4.write(l+2, nb_var*h+1+11, SSInx_m_w[h,l])
            ...
            ws_56.write(l+2, nb_var*h+1+3,  Qs_ot_m_w[h,l]/A_2_mm_w[h])
            ws_56.write(l+2, nb_var*h+1+4,  Qo_in_m_w[h,l]/A_2_mm_w[h])
            ws_56.write(l+2, nb_var*h+1+5,  Qo_ot_m_w[h,l]/A_2_mm_w[h])
            ws_56.write(l+2, nb_var*h+1+6,  Qs_in_m_r[h,l]/A_2_mm_w[h])
            ws_56.write(l+2, nb_var*h+1+7,  Qo_in_m_r[h,l]/A_2_mm_w[h])
            ws_56.write(l+2, nb_var*h+1+8,  Pt_m_w[h,l]/A_2_mm_w[h])
            ws_56.write(l+2, nb_var*h+1+9,  wb_tmp_m/A_2_mm_w[h])
            if l:
                ws_56.write_formula(l+2, nb_var*h+1+10, er_f)
            ws_56.write(l+2, nb_var*h+1+11, SSInx_m_w[h,l])

        # Write WWB yearly data - absolute volumes
        for l in xrange(len(D_yer)):
            wb_tmp_y = Pt_y_w[h,l]-float(Ea_y_w[h,l])-Qs_ot_y_w[h,l]+\
            Qs_in_y_w[h,l]-Qo_ot_y_w[h,l]+Qo_in_y_w[h,l]
            if h == 0:
            ...
            ws_5.write(l+2, nb_var*h+1+2,  Qs_in_y_w[h,l])
            ws_5.write(l+2, nb_var*h+1+3,  Qs_ot_y_w[h,l])
            ws_5.write(l+2, nb_var*h+1+4,  Qo_in_y_w[h,l])
            ws_5.write(l+2, nb_var*h+1+5,  Qo_ot_y_w[h,l])
            ws_5.write(l+2, nb_var*h+1+6,  Qs_in_y_r[h,l])
            ws_5.write(l+2, nb_var*h+1+7,  Qo_in_y_r[h,l])
            ws_5.write(l+2, nb_var*h+1+8,  Pt_y_w[h,l])
            ws_5.write(l+2, nb_var*h+1+9,  wb_tmp_y)
            if l:
                ws_5.write_formula(l+2, nb_var*h+1+10, er_f)
            ws_5.write(l+2, nb_var*h+1+11, SSInx_y_w[h,l])
            ...
            ws_57.write(l+2, nb_var*h+1+2,  Qs_in_y_w[h,l]/A_2_mm_w[h])
            ws_57.write(l+2, nb_var*h+1+3,  Qs_ot_y_w[h,l]/A_2_mm_w[h])
            ws_57.write(l+2, nb_var*h+1+4,  Qo_in_y_w[h,l]/A_2_mm_w[h])
            ws_57.write(l+2, nb_var*h+1+5,  Qo_ot_y_w[h,l]/A_2_mm_w[h])
            ws_57.write(l+2, nb_var*h+1+6,  Qs_in_y_r[h,l]/A_2_mm_w[h])
            ws_57.write(l+2, nb_var*h+1+7,  Qo_in_y_r[h,l]/A_2_mm_w[h])
            ws_57.write(l+2, nb_var*h+1+8,  Pt_y_w[h,l]/A_2_mm_w[h])
            ws_57.write(l+2, nb_var*h+1+9,  wb_tmp_y/A_2_mm_w[h])
            if l:
                ws_57.write_formula(l+2, nb_var*h+1+10, er_f)
            ws_57.write(l+2, nb_var*h+1+11, SSInx_y_w[h,l])
...
#        print 'cnt_y:', cnt_y
        if cnt_y:
            ...
            -np.average(Qs_ot_y_w[h,:])+np.average(Qs_in_y_w[h,:])\
            ...
            ws_51.write(h+2, 3,  np.average(Qs_in_y_w[h,:]))
            wwb_y_tmp_m3.append(np.average(Qs_in_y_w[h,:]))
            ws_51.write(h+2, 4,  np.average(Qs_ot_y_w[h,:]))
            wwb_y_tmp_m3.append(np.average(Qs_ot_y_w[h,:]))
            ws_51.write(h+2, 5,  np.average(Qo_in_y_w[h,:]))
            wwb_y_tmp_m3.append(np.average(Qo_in_y_w[h,:]))
            ws_51.write(h+2, 6,  np.average(Qo_ot_y_w[h,:]))
            wwb_y_tmp_m3.append(np.average(Qo_ot_y_w[h,:]))
            ws_51.write(h+2, 7,  np.average(Qs_in_y_r[h,:]))
            wwb_y_tmp_m3.append(np.average(Qs_in_y_r[h,:]))
            ws_51.write(h+2, 8,  np.average(Qo_in_y_r[h,:]))
            wwb_y_tmp_m3.append(np.average(Qo_in_y_r[h,:]))
            ws_51.write(h+2, 9,  np.average(Pt_y_w[h,:]))
            wwb_y_tmp_m3.append(np.average(Pt_y_w[h,:]))
            ws_51.write(h+2, 10, wb_tmp_y_avg)
            wwb_y_tmp_m3.append(wb_tmp_y_avg)
            if l:
                ws_51.write(h+2, 11, 'NaN')
            ws_51.write(h+2, 12, np.average(SSInx_y_w[h,:]))
            wwb_y_tmp_m3.append(np.average(SSInx_y_w[h,:]))
            ws_51.write(h+2, 13, cnt_y)
            wwb_y_tmp_m3.append(cnt_y)
            ws_51.write(h+2, 14, run_name)
            ...
            ws_52.write(h+2, 3,  np.average(Qs_in_y_w[h,:])/A_2_mm_w[h])
            wwb_y_tmp_mm.append(np.average(Qs_in_y_w[h,:])/A_2_mm_w[h])
            ws_52.write(h+2, 4,  np.average(Qs_ot_y_w[h,:])/A_2_mm_w[h])
            wwb_y_tmp_mm.append(np.average(Qs_ot_y_w[h,:])/A_2_mm_w[h])
            ws_52.write(h+2, 5,  np.average(Qo_in_y_w[h,:])/A_2_mm_w[h])
            wwb_y_tmp_mm.append(np.average(Qo_in_y_w[h,:])/A_2_mm_w[h])
            ws_52.write(h+2, 6,  np.average(Qo_ot_y_w[h,:])/A_2_mm_w[h])
            wwb_y_tmp_mm.append(np.average(Qo_ot_y_w[h,:])/A_2_mm_w[h])
            ws_52.write(h+2, 7,  np.average(Qs_in_y_r[h,:])/A_2_mm_w[h])
            wwb_y_tmp_mm.append(np.average(Qs_in_y_r[h,:])/A_2_mm_w[h])
            ws_52.write(h+2, 8,  np.average(Qo_in_y_r[h,:])/A_2_mm_w[h])
            wwb_y_tmp_mm.append(np.average(Qo_in_y_r[h,:])/A_2_mm_w[h])
            ws_52.write(h+2, 9,  np.average(Pt_y_w[h,:])/A_2_mm_w[h])
            wwb_y_tmp_mm.append(np.average(Pt_y_w[h,:])/A_2_mm_w[h])
            ws_52.write(h+2, 10, wb_tmp_y_avg/A_2_mm_w[h])
            wwb_y_tmp_mm.append(wb_tmp_y_avg/A_2_mm_w[h])
            if l:
                ws_52.write(h+2, 11, 'NaN')
            ws_52.write(h+2, 12, np.average(SSInx_y_w[h,:]))
            wwb_y_tmp_mm.append(np.average(SSInx_y_w[h,:]))
            ws_52.write(h+2, 13, cnt_y)
            wwb_y_tmp_mm.append(cnt_y)
            ws_52.write(h+2, 14, run_name)
...
def WWBU_post_processing_DL(plot_ini='plot-flow-precip.ini',
                            start_stamp='', split_month=10,
                            topkapi_ini = 'TOPKAPI.ini',
                            ver_ini = 'run_version.ini',
                            WWBU_transfer_n2i = False):
    """ Wetland Water Balance Units processing:
    ...
    A flow duration curve based on monthly values is created.
    Wetland water balance units will be calculated and outputs will be
    saved in an XLS-file.
    Also sets up a database of for SSI and yearly water balance components
    and saves it as a numpy array binary file.
    Do do: update for second soil layer.
...
#==============================================================================
    ...
    # Filter rivers out of WWB cells
    ar_wwb[ar_lambda==1] = -9999.
    ...
    ET_out_w  = np.array(ET_out_w)    # m3/d
    Vs_cel_w  = np.array(Vs_cel_w)    # m3
    Vs_cel_w_ = np.array(Vs_cel_w_)   # m3
    Rainfl_w  = np.array(Rainfl_w)    # m3/d
    SSIndx_w  = np.array(SSIndx_w)    # Dimensionless
    SSIndx_w_ = np.array(SSIndx_w_)   # Dimensionless
    wwbu_A_w  = np.array(wwbu_A_w)    # m2
...
# To do: use the 'pyshape module to write shapefiles directly instead of
# cluttering the XLS file.
...
# To do: make a new piezometer routine for two soil layers
...
# Writing daily basic catchment data in sheet P_Qsim
    ...
    #===========================================================================
    # Monthly averages from the flow
    #===========================================================================
...
#==============================================================================
    ...
    ws_hdr = ['ETa','Vs','Vs_II','Qs_in','Qs_in_II','Qs_out','Qs_out_II',
              'Qo_in','Qo_out','Qs_in_r','Qs_in_r_II','Qo_in_r','Rain','WB',
              'ERR','SSI','SSI_II']
    um3    = ['(m3/','(m3)','(m3)','(m3/','(m3/','(m3/','(m3/','(m3/','(m3/',
              '(m3/','(m3/','(m3/','(m3/','(m3/','(m3/','-','-']
    umm    = ['(mm/','(mm)','(mm)','(mm/','(mm/','(mm/','(mm/','(mm/','(mm/',
              '(mm/','(mm/','(mm/','(mm/','(mm/','(mm/','-','-']
    ...
    er_f = '=(INDIRECT(ADDRESS(ROW(),COLUMN()-13))-INDIRECT(ADDRESS(ROW()-1,\
COLUMN()-13)))+(INDIRECT(ADDRESS(ROW(),COLUMN()-12))-INDIRECT(ADDRESS(ROW()-1,\
COLUMN()-12)))-INDIRECT(ADDRESS(ROW(),COLUMN()-1))'
    ...
    for h in xrange(nb_wwb):
        ...
        for l in xrange(n_rec):
            ...
            ws_3.write(l+2, nb_var*h+1+0,  float(ET_out_w[h,l]))
            ws_3.write(l+2, nb_var*h+1+1,  float(Vs_cel_w[h,l]))
            ws_3.write(l+2, nb_var*h+1+2,  float(Vs_cel_w_[h,l]))
            ws_3.write(l+2, nb_var*h+1+3,  Qs_in_w[h,l])
            ws_3.write(l+2, nb_var*h+1+4,  Qs_in_w_[h,l])
            ws_3.write(l+2, nb_var*h+1+5,  Qs_ot_w[h,l])
            ws_3.write(l+2, nb_var*h+1+6,  Qs_ot_w_[h,l])
            ws_3.write(l+2, nb_var*h+1+7,  Qo_in_w[h,l])
            ws_3.write(l+2, nb_var*h+1+8,  Qo_ot_w[h,l])
            ws_3.write(l+2, nb_var*h+1+9,  Qs_in_r[h,l])
            ws_3.write(l+2, nb_var*h+1+10,  Qs_in_r_[h,l])
            ws_3.write(l+2, nb_var*h+1+11, Qo_in_r[h,l])
            ws_3.write(l+2, nb_var*h+1+12, float(Rainfl_w[h,l]))
            ws_3.write(l+2, nb_var*h+1+13, wb_tmp_d)
            if l:
                ws_3.write_formula(l+2, nb_var*h+1+14, er_f)
            ws_3.write(l+2, nb_var*h+1+15, SSIndx_w[h,l])
            ws_3.write(l+2, nb_var*h+1+16, SSIndx_w_[h,l])
            ...
            ws_55.write(l+2, nb_var*h+1+0,  float(ET_out_w[h,l])/A_2_mm_w[h])
            ws_55.write(l+2, nb_var*h+1+1,  float(Vs_cel_w[h,l])/A_2_mm_w[h])
            ws_55.write(l+2, nb_var*h+1+2,  float(Vs_cel_w_[h,l])/A_2_mm_w[h])
            ws_55.write(l+2, nb_var*h+1+3,  Qs_in_w[h,l]/A_2_mm_w[h])
            ws_55.write(l+2, nb_var*h+1+4,  Qs_in_w_[h,l]/A_2_mm_w[h])
            ws_55.write(l+2, nb_var*h+1+5,  Qs_ot_w[h,l]/A_2_mm_w[h])
            ws_55.write(l+2, nb_var*h+1+6,  Qs_ot_w_[h,l]/A_2_mm_w[h])
            ws_55.write(l+2, nb_var*h+1+7,  Qo_in_w[h,l]/A_2_mm_w[h])
            ws_55.write(l+2, nb_var*h+1+8,  Qo_ot_w[h,l]/A_2_mm_w[h])
            ws_55.write(l+2, nb_var*h+1+9,  Qs_in_r[h,l]/A_2_mm_w[h])
            ws_55.write(l+2, nb_var*h+1+10, Qs_in_r_[h,l]/A_2_mm_w[h])
            ws_55.write(l+2, nb_var*h+1+11, Qo_in_r[h,l]/A_2_mm_w[h])
            ws_55.write(l+2, nb_var*h+1+12, float(Rainfl_w[h,l])/A_2_mm_w[h])
            ws_55.write(l+2, nb_var*h+1+13,  wb_tmp_d/A_2_mm_w[h])
            if l:
                ws_55.write_formula(l+2, nb_var*h+1+14, er_f)
            ws_55.write(l+2, nb_var*h+1+15, SSIndx_w[h,l])
            ws_55.write(l+2, nb_var*h+1+16, SSIndx_w_[h,l])

        # Write WWB monthly data - absolute volumes
        for l in xrange(len(D_mon)):
            wb_tmp_m = Pt_m_w[h,l]-float(Ea_m_w[h,l])-Qs_ot_m_w[h,l]-\
            Qs_ot_m_w[h,l]+Qs_in_m_w[h,l]+Qs_in_m_w_[h,l]-Qo_ot_m_w[h,l]+\
            Qo_in_m_w[h,l]
            if h == 0:
                ws_4.write(l+2,        0,  D_mon[l], date_format)
            ws_4.write(l+2, nb_var*h+1+0,  float(Ea_m_w[h,l]))
            ws_4.write(l+2, nb_var*h+1+1,  float(Vs_m_w[h,l]))
            ws_4.write(l+2, nb_var*h+1+2,  float(Vs_m_w[h,l]))
            ws_4.write(l+2, nb_var*h+1+3,  Qs_in_m_w[h,l])
            ws_4.write(l+2, nb_var*h+1+4,  Qs_in_m_w_[h,l])
            ws_4.write(l+2, nb_var*h+1+5,  Qs_ot_m_w[h,l])
            ws_4.write(l+2, nb_var*h+1+6,  Qs_ot_m_w_[h,l])
            ws_4.write(l+2, nb_var*h+1+7,  Qo_in_m_w[h,l])
            ws_4.write(l+2, nb_var*h+1+8,  Qo_ot_m_w[h,l])
            ws_4.write(l+2, nb_var*h+1+9,  Qs_in_m_r[h,l])
            ws_4.write(l+2, nb_var*h+1+10, Qs_in_m_r_[h,l])
            ws_4.write(l+2, nb_var*h+1+11, Qo_in_m_r[h,l])
            ws_4.write(l+2, nb_var*h+1+12, Pt_m_w[h,l])
            ws_4.write(l+2, nb_var*h+1+13, wb_tmp_m)
            if l:
                ws_4.write_formula(l+2, nb_var*h+1+14, er_f)
            ws_4.write(l+2, nb_var*h+1+15, SSInx_m_w[h,l])
            ws_4.write(l+2, nb_var*h+1+16, SSInx_m_w_[h,l])
            ...
            ws_56.write(l+2, nb_var*h+1+0,  float(Ea_m_w[h,l])/A_2_mm_w[h])
            ws_56.write(l+2, nb_var*h+1+1,  float(Vs_m_w[h,l])/A_2_mm_w[h])
            ws_56.write(l+2, nb_var*h+1+2,  float(Vs_m_w_[h,l])/A_2_mm_w[h])
            ws_56.write(l+2, nb_var*h+1+3,  Qs_in_m_w[h,l]/A_2_mm_w[h])
            ws_56.write(l+2, nb_var*h+1+4,  Qs_in_m_w_[h,l]/A_2_mm_w[h])
            ws_56.write(l+2, nb_var*h+1+5,  Qs_ot_m_w[h,l]/A_2_mm_w[h])
            ws_56.write(l+2, nb_var*h+1+6,  Qs_ot_m_w_[h,l]/A_2_mm_w[h])
            ws_56.write(l+2, nb_var*h+1+7,  Qo_in_m_w[h,l]/A_2_mm_w[h])
            ws_56.write(l+2, nb_var*h+1+8,  Qo_ot_m_w[h,l]/A_2_mm_w[h])
            ws_56.write(l+2, nb_var*h+1+9,  Qs_in_m_r[h,l]/A_2_mm_w[h])
            ws_56.write(l+2, nb_var*h+1+10, Qs_in_m_r_[h,l]/A_2_mm_w[h])
            ws_56.write(l+2, nb_var*h+1+11, Qo_in_m_r[h,l]/A_2_mm_w[h])
            ws_56.write(l+2, nb_var*h+1+12, Pt_m_w[h,l]/A_2_mm_w[h])
            ws_56.write(l+2, nb_var*h+1+13, wb_tmp_m/A_2_mm_w[h])
            if l:
                ws_56.write_formula(l+2, nb_var*h+1+14, er_f)
            ws_56.write(l+2, nb_var*h+1+15, SSInx_m_w[h,l])
            ws_56.write(l+2, nb_var*h+1+16, SSInx_m_w_[h,l])
        ...
        for l in xrange(len(D_yer)):
            ...
            ws_5.write(l+2, nb_var*h+1+16, SSInx_y_w_[h,l])
            ...
            ws_57.write(l+2, nb_var*h+1+16, SSInx_y_w_[h,l])
