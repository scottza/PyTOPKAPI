...
def read_cell_parameters_DL(file_name):
    """Read the spatially variable cell parameters from file.
    ...
    ar_Ks0_ : (N,) float array
        The saturated hydraulic conductivity of the deep layer
        (:math:`mm/s`)
    ar_theta_r_ : (N,) float array
        The residual soil moisture content for the deeper layer
    ar_theta_s_ : (N,) float array
        The saturated soil moisture content for the deeper layer
    ar_pVs_t0_ : (N,) float array
        The initial saturation of  the deeper layer
    psi_b_ : (N,) float array
        The air entry value of the deeper layer
    lamda_ : (N,) float array
        The Brooks and Corey pore size distribution index for the deeper layer
    ...
    ar_L0_              = tab_read[:,21]
    ar_Ks0_             = tab_read[:,22]
    ar_theta_r_         = tab_read[:,23]
    ar_theta_s_         = tab_read[:,24]
    ar_pVs_t0_          = tab_read[:,25]
    psi_b_              = tab_read[:,26]
    lamda_              = tab_read[:,27]
    ...
    return ar_cell_label, ar_coorx,ar_coory, ar_lambda, \
           ...
           ar_n_o, ar_n_c, ar_cell_down, \
           ar_pVs_t0, ar_Vo_t0, ar_Qc_t0, ar_kc, psi_b, lamda,\
           ar_L0_, ar_Ks0_, ar_theta_r_, ar_theta_s_, ar_pVs_t0_, \
           psi_b_, lamda_
...
* (ar_A_drained**0.5-A_total**0.5)'
            ...
            print 'ar_A_drained', ar_A_drained[c]
            err_txt = 'The equation to approximite channel width (W) has \
produced negative values. This is most likely due to a very large value of \
\'A_thres\' defined in the global parameter file. \'A_thres\' should be at \
least 20 times smaller than the total catchment area (\'A_total\'), currently \
it is approx. %i times smaller. The input parameters for calculating W are \
printed above.' % int(A_total/A_thres)
...
def compute_cell_param_DL(X, ar_Xc, Dt, alpha_s, alpha_o,
                       alpha_c, nb_cell, A_thres, W_max,
                       W_min, ar_lambda, ar_tan_beta,
                       ar_tan_beta_channel, ar_L, ar_Ks,
                       ar_theta_r, ar_theta_s, ar_n_o,
                       ar_n_c, ar_A_drained,
                       ar_L_, ar_Ks_, ar_theta_r_,
                       ar_theta_s_):
    """Compute model parameters from physical parameters.
    ...
    ar_A_drained : (N,) float array
        The total drained area associated with each cell (:math:`m^2`)
    ar_L_ : (N,) float array
        The depth of the soil store for the second layer (:math:`m`)
    ar_Ks_ : (N,) float array
        The saturated hydraulic conductivity of the second layer
        (:math:`mm/s`)
    ar_theta_r_ : (N,) float array
        The residual soil moisture content for the second layer
    ar_theta_s_ : (N,) float array
        The saturated soil moisture content for the second layer
    ...
    ##Soil parameters
    ar_Vsm  = (ar_theta_s -ar_theta_r )*(X**2)*ar_L
    ar_Vsm_ = (ar_theta_s_-ar_theta_r_)*(X**2)*ar_L_
    ar_Cs   = (ar_L *ar_Ks *ar_tan_beta)/(((ar_theta_s -ar_theta_r )*ar_L )**alpha_s)
    ar_Cs_  = (ar_L_*ar_Ks_*ar_tan_beta)/(((ar_theta_s_-ar_theta_r_)*ar_L_)**alpha_s)
    ar_b_s  = ar_Cs *X/(X**(2*alpha_s))
    ar_b_s_ = ar_Cs_*X/(X**(2*alpha_s))
    ##Overland parameters
    ar_Co   = (1/ar_n_o)*(ar_tan_beta)**0.5
    ar_b_o  = ar_Co*X/(X**(2*alpha_o))
    ##Channel parameters
...
printed above.' % int(A_total/A_thres)
    ...
    return ar_Vsm, ar_b_s, ar_b_o, ar_W, ar_b_c, ar_Vsm_, ar_b_s_
